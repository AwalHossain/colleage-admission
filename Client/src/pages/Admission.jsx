...
const Admission = () => {
  ...
  const mutation = useMutation(submitAdmission,{
      onSuccess:() => {
        ...
        navigation.navigate("/my-college");
        
        },
        onError: (error) => {
          console.log(error,"checking from");
  ...
  const {isLoading:MutateLoading, error,data,isSuccess} = mutation;
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          setToLocalStorage("image", response.data.data.display_url);
        } else {
          setLoading(false);
          toast.error("Error: " + "Something went wrong while uploading image");
  ...
  const handleSubmit = async (e) => {
    ...
    mutation.mutate(formData)
      ...
      console.error("Error: from sadmiadsfo", err.response.data);
      toast.error(err.response.data.message || err.message);
    }
  };

  useEffect(() => {
    const image = getFromLocalStorage("image");
    if (image) {
      setImageUrl(image);
